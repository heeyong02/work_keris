...
uploaded_file1 = st.sidebar.file_uploader(
    "학교별 교육편제단위 정보 파일 (Excel)", type=["xlsx"], key="file1"
)

uploaded_file2 = st.sidebar.file_uploader(
    "교육과정_대학 파일 (Excel)", type=["xlsx"], key="file2"
)
...
if uploaded_file1 and uploaded_file2:
    try:
        ...
        with st.spinner("📊 데이터를 로딩하는 중..."):
            # 데이터 로드
            df1 = pd.read_excel(uploaded_file1, skiprows=4)
            df2 = pd.read_excel(uploaded_file2, skiprows=8)
        ...
        education_program_correction_time = datetime.now() - education_program_correction_start_time
        logger.info(f"교육과정 이름 정제 완료 시간: {education_program_correction_time}")
        ...
        course_distribution_calculation_time = datetime.now() - course_distribution_calculation_start_time
        logger.info(f"교육과정별 분류 비율 계산 완료 시간: {course_distribution_calculation_time}")
        ...
        recommendation_result_generation_time = datetime.now() - recommendation_result_generation_start_time
        logger.info(f"추천 결과 생성 완료 시간: {recommendation_result_generation_time}")
        ...
        major_recommendation_generation_time = datetime.now() - major_recommendation_generation_start_time
        logger.info(f"전공별 추천 결과 생성 완료 시간: {major_recommendation_generation_time}")
